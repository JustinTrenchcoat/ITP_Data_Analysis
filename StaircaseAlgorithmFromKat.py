...
for t in range(time_dim):
    ...
    if np.sum(~np.isnan(temp_profile)) > 10:
        valid_indices = np.where((depth_profile >= 200) & (depth_profile <= 600))[0]
        if len(valid_indices) == 0:
            continue  

        max_temp_index = valid_indices[np.argmax(temp_profile[valid_indices])]

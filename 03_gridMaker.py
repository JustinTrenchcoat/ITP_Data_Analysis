...
def prep(full_path, error_list):
    with h5py.File(full_path, 'r') as f:
        ...
        # maybe we don't need exact time
        ...
        # select only from Tmin (above 400m?)to 2+Tmax for interpolation:
        # surface_index is all indeces of depth array who's above T_Max and below 100m
        surface_index = np.where((120 <= depths_sorted) & (depths_sorted <= T_Max_Depth))[0]
    ...
    return regular_depths, interpolated_temperatures,interpolated_salinity,lat, lon,psdate,pedate,pstart,pstop

def makeGrid(full_path, file_name, folder_name):
    regular_depths, interpolated_temperatures,interpolated_salinity,lat, lon,psdate,pedate,pstart,pstop = prep(full_path, error_list)
    # Create DataFrame
    df = pd.DataFrame({
    ...
    df['latitude'] = lat[0].round(4)
    df['longitude'] = lon[0].round(4)
...
#########################################################################################
# make matlab file version of gridded datasets
mat_error_list = []
def makeMatGrid(full_path, file_name, folder_name):
    regular_depths, interpolated_temperatures,interpolated_salinity,lat, lon,psdate,pedate,pstart,pstop = prep(full_path, mat_error_list)
...
###########################################################################################
# call it when you need to make gridded data
# # make csv files 

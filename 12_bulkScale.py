...
def boxPlots(df):
    ...
    def dTdZ(df):
        return dT(df)/dZ(df)
    singleBoxplot(df, dTdZ, "Bulk-Scale Temperature Gradient", "Celcius/m", (0.014,0.005),"dTdZBox")

    def dSdZ(df):
        return dS(df)/dZ(df)
    singleBoxplot(df, dSdZ, "Bulk-Scale Salinity Gradient", "(g/kg)/m", (0.011,0.004), "dSdZBox")
